...
export default function GasNaturalProjectV2() {
  ...
  // V10.4 — User plan + capabilities (loaded once via /api/me/plan)
  const { plan: userPlan, refresh: refreshPlan } = useUserPlan();
  const caps = userPlan?.capabilities || {};
  // Helpful aliases for gating
  const canGenerateDocs = caps.can_generate_docs !== false;  // default-allow if plan not yet loaded
  const canDispatchEmails = caps.can_dispatch_emails !== false;
  const canTransfer = caps.can_transfer !== false;
  const canExport = caps.can_export !== false;
  const docsLimitReached = userPlan?.usage?.documents_remaining === 0;
  const projectsLimitReached = userPlan?.usage?.projects_remaining === 0;
  ...
  const downloadDoc = async (templateId, filename) => {
    ...
    try {
      ...
      if (!res.ok) {
        // Try to extract backend error message (quota / capability errors)
        let errMsg = `HTTP ${res.status}`;
        try { const j = await res.json(); if (j.detail) errMsg = j.detail; } catch (_) { /* ignore parse errors */ }
      ...
      refreshPlan?.();  // V10.4 — refresh usage counter
    } catch (e) {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
        ...
        <div className="xl:col-span-9 space-y-4">
          ...
          <SectionCard id="generare-documente" title="Generare documente DOCX (Deviz · Situație · Listă materiale · Fișă tehnică)" collapsed={collapsed['generare-documente']} onToggle={() => toggleSection('generare-documente')}>
            {!canGenerateDocs && (
              <div className="mb-3 p-3 bg-rose-50 border border-rose-200 rounded-lg flex items-start gap-2 text-xs text-rose-800" data-testid="docs-locked-notice">
                <ShieldCheck className="w-4 h-4 mt-0.5 shrink-0" />
                <div>
                  <strong>Planul «{userPlan?.plan_name}» nu permite generare documente.</strong> Upgradează planul (ex: Operator/Proiectant) pentru a debloca această funcție.
                </div>
              </div>
            )}
            {docsLimitReached && canGenerateDocs && (
              <div className="mb-3 p-3 bg-rose-50 border border-rose-200 rounded-lg flex items-start gap-2 text-xs text-rose-800" data-testid="docs-quota-reached-notice">
                <ShieldCheck className="w-4 h-4 mt-0.5 shrink-0" />
                <div>
                  <strong>Cotă lunară atinsă</strong> ({userPlan?.documents_per_month} documente / lună pentru planul «{userPlan?.plan_name}»). Upgradează planul.
                </div>
              </div>
            )}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {GENERATE_DOCS.map((g) => {
                const blocked = !canGenerateDocs || docsLimitReached;
                return (
                  <button
                    key={g.id}
                    onClick={() => { if (!blocked) downloadDoc(g.template, `${g.id}_${pid}.docx`); }}
                    disabled={blocked}
                    className={`group relative overflow-hidden bg-white border rounded-lg p-4 text-left transition-all ${blocked ? 'border-slate-200 opacity-50 cursor-not-allowed' : 'border-slate-200 hover:border-violet-400 hover:shadow-lg'}`}
                    data-testid={`generate-${g.id}`}
                    title={blocked ? 'Funcție blocată — upgradează planul' : `Generează ${g.label}`}
                  >
                    {!blocked && <div className="absolute inset-0 bg-gradient-to-br from-violet-50/0 to-violet-50/0 group-hover:from-violet-50/40 group-hover:to-indigo-50/30 transition-all"></div>}
                    <FileText className={`w-5 h-5 mb-2 relative ${blocked ? 'text-slate-400' : 'text-violet-600'}`} />
                    <div className={`text-xs font-bold relative ${blocked ? 'text-slate-500' : 'text-slate-900'}`}>{g.label}</div>
                    <div className={`text-[10px] mt-1 relative font-semibold flex items-center gap-1 ${blocked ? 'text-slate-400' : 'text-violet-600'}`}>
                      {blocked ? '🔒 Blocat' : <>Generează DOCX <Download className="w-2.5 h-2.5" /></>}
                    </div>
                  </button>
                );
              })}
        ...
        <aside className="xl:col-span-3 space-y-4">
          ...
          {/* V10.4 — Plan badge with live usage (proiecte/luna + documente/luna) */}
                ...
                <div key={r.id} className="text-[10px]">
                  ...
                  <div className="flex gap-1">
                    ...
                    <button
                      ...
                      disabled={!data.emails?.[r.id] || dispatchingRoute === r.id || !canDispatchEmails}
                      ...
                      title={!canDispatchEmails ? 'Planul tău nu permite trimitere email automată' : ''}
                    >
                      {dispatchingRoute === r.id ? <Loader2 className="w-3 h-3 animate-spin" /> : (canDispatchEmails ? 'Trimite' : '🔒')}
                    </button>
                  </div>
                </div>
              ))}
              ...
              {FINAL_DOWNLOADS.map((d) => {
                ...
                return (
                  <div key={d.id} className="flex items-center justify-between text-xs bg-slate-50/60 rounded p-2">
                    <span className={`font-semibold ${blocked ? 'text-slate-400' : 'text-slate-800'}`}>{d.label}</span>
                    <button
                      onClick={() => { if (!blocked) downloadDoc(d.template, `${d.id}_${pid}.docx`); }}
                      disabled={blocked}
                      className="bg-blue-600 text-white px-2.5 py-1 text-[10px] inline-flex items-center gap-1 hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed rounded font-semibold transition-colors"
                      data-testid={`download-${d.id}`}
                      title={blocked ? 'Funcție blocată — upgrade plan' : ''}
                    >
                      {blocked ? '🔒' : <><Download className="w-3 h-3" /> DOCX</>}
                    </button>
                  </div>
                );
              })}
          ...
          <div className="bg-white border border-violet-200 rounded-xl epd-shadow overflow-hidden" data-testid="final-actions-panel">
            ...
            <div className="p-3 space-y-2">
              ...
              <button
                ...
                disabled={!canExport || docsLimitReached}
                className="w-full text-xs inline-flex items-center justify-center gap-2 bg-slate-900 text-white px-3 py-2.5 hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed rounded-md font-semibold transition-colors"
                data-testid="download-dossier-complet"
                title={!canExport ? 'Planul tău nu permite export ZIP' : (docsLimitReached ? 'Cotă lunară de documente atinsă' : '')}
              >
                {!canExport ? '🔒' : <Package className="w-3.5 h-3.5" />} Descarcă dosar ZIP
              </button>
              <button
                onClick={() => window.open(`${backendUrl}/api/gas-project/${pid}/dossier.zip`, '_blank')}
                disabled={!canExport}
                className="w-full text-xs inline-flex items-center justify-center gap-2 bg-white text-violet-700 border border-violet-300 hover:bg-violet-50 disabled:opacity-40 disabled:cursor-not-allowed px-3 py-2.5 rounded-md font-semibold transition-colors"
                data-testid="preview-dossier"
              >
                <Eye className="w-3.5 h-3.5" /> Previzualizează dosar
              </button>
              <button
                onClick={() => { if (canTransfer) setShowTransfer(true); else toast.error('Planul tău nu permite transfer'); }}
                disabled={!canTransfer}
                className="w-full text-xs inline-flex items-center justify-center gap-2 bg-white text-indigo-700 border border-indigo-300 hover:bg-indigo-50 disabled:opacity-40 disabled:cursor-not-allowed px-3 py-2.5 rounded-md font-semibold transition-colors"
                data-testid="open-transfer-dialog"
                title={!canTransfer ? 'Planul tău nu permite transfer către alți utilizatori' : ''}
              >
                <Send className="w-3.5 h-3.5 rotate-90" /> {canTransfer ? 'Transferă către alt utilizator' : '🔒 Transfer blocat'}
              </button>
            </div>
