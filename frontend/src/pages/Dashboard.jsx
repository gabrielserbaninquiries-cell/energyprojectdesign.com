...
export default function Dashboard() {
  ...
  return (
    <AppShell title={`Bun venit, ${user?.name?.split(' ')[0] || ''}`}>
      {/* Premium hero header */}
      <div className="relative mb-8 overflow-hidden rounded-2xl bg-zinc-950 p-8 text-white shadow-2xl" data-testid="dashboard-hero">
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-6 items-center">
          <div>
            <div className="text-[10px] uppercase tracking-[0.3em] text-zinc-300 mb-2">// Panou principal · V11.0</div>
            <h1 className="text-3xl sm:text-4xl font-bold tracking-tight mb-2">
              Bun venit, <span className="text-zinc-300">{user?.name?.split(' ')[0] || 'inginer'}</span>.
            </h1>
            <p className="text-zinc-300/90 text-sm max-w-2xl">
            ...
            <div className="mt-5 flex items-center gap-2 flex-wrap">
              <Link to="/gaze-naturale" className="px-4 py-2 bg-white text-zinc-950 hover:bg-zinc-50 rounded-lg font-semibold text-sm flex items-center gap-2 transition-all shadow-lg" data-testid="dashboard-cta-gas">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8" data-testid="dashboard-stats">
        {stats.map((s) => {
          const Icon = s.icon;
          return (
            <Link to={s.to} key={s.label} className="group p-5 bg-white border border-slate-200 rounded-xl hover:border-zinc-950 hover:shadow-lg transition-all hover-lift" data-testid={`stat-${s.label}`}>
              <div className="flex items-start justify-between mb-3">
                ...
                <ArrowRight className="w-4 h-4 text-slate-300 group-hover:text-zinc-600 group-hover:translate-x-1 transition-all" />
              </div>
      ...
      <div className="mb-8">
        <div className="flex items-end justify-between mb-4">
          <div>
            <div className="text-[10px] uppercase tracking-[0.25em] text-zinc-600 font-semibold mb-1">// Acțiuni rapide</div>
            <h2 className="text-xl font-bold text-slate-900">Începe o lucrare nouă</h2>
          </div>
          <Link to="/industrii" className="text-xs text-zinc-600 hover:text-zinc-950 font-semibold flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" data-testid="dashboard-quick-actions">
          {quickActions.map((q) => {
            const Icon = q.icon;
            return (
              <Link to={q.to} key={q.label} className="group p-5 bg-white border border-slate-200 rounded-xl hover:border-zinc-950 hover:shadow-lg transition-all" data-testid={`quick-${q.label.replace(/\s+/g, '-').toLowerCase()}`}>
                ...
                <div className="mt-3 text-xs text-zinc-600 font-semibold flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {lockedFeatureCount > 0 && (
          <Link
            to="/pricing"
            className="mt-4 flex items-center justify-between gap-4 p-4 bg-zinc-50 border border-zinc-200 rounded-xl hover:border-zinc-950 hover:shadow-md transition-all group"
            data-testid="upgrade-cta-locked"
          >
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-gradient-to-br bg-zinc-950 /* was gradient */ flex items-center justify-center shadow-md">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
            ...
            <Link to="/documents" className="text-xs uppercase tracking-wider text-zinc-600 hover:text-zinc-950 font-semibold">Toate →</Link>
          </div>
          {recent.length === 0 ? (
            ...
            <ul className="divide-y divide-slate-100">
              {recent.map((d) => (
                <li key={d.document_id} className="px-6 py-3.5 hover:bg-slate-50 flex items-center justify-between transition-colors" data-testid={`recent-doc-${d.document_id}`}>
                  ...
                  <Link to="/documents" className="text-xs text-zinc-600 hover:text-zinc-950 font-semibold">Deschide →</Link>
                </li>
              ))}
            </ul>
          )}
        ...
        <div className="relative bg-slate-900 text-white rounded-xl overflow-hidden p-6">
          ...
          <div className="relative">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] uppercase tracking-[0.2em] text-zinc-400 font-bold">// Plan curent</span>
            </div>
            ...
            {user?.plan === 'free' && (
              <Link to="/pricing" className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-zinc-950 hover:bg-zinc-800 rounded-lg font-semibold text-sm transition-all" data-testid="upgrade-btn">
